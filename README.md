...
If you enter an optional or hidden area that you do not want in the displayed run metrics, open the pause menu in that area and enable `Exclude AREA NAME from Metrics`. The area is treated like `Unknown` for PB, area number, area name, and screen summary output. The raw area data is still kept internally, so disabling the option restores it to the displayed metrics. In an `Unknown` area, the option is shown as checked and cannot be changed because that area is already excluded by definition.
